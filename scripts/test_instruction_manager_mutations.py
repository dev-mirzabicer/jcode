...
try:
    ...
    for cols,rows in [(150,40),(80,24),(60,24),(24,10)]:
        tid=spawn_client(session,cols,rows);keys(tid,'/,u,i,-,m,o,d,u,l,e,enter');f.tester_state(tid,lambda state:state.get('rows_loaded')==1 and state.get('pending_id') is None);keys(tid,'ctrl+e');ui(tid,lambda state:state.get('draft') and state.get('pending') is None)
        mode.write_text('fail');keys(tid,'b');ui(tid,lambda state:state.get('failed') and state.get('pending') is None)
        ...
        keys(tid,'r');ui(tid,lambda state:state.get('reviewed') and state.get('pending') is None)
    ...
    tid=spawn_client(session,80,24);keys(tid,'/,u,i,-,m,o,d,u,l,e,enter');f.tester_state(tid,lambda state:state.get('rows_loaded')==1 and state.get('pending_id') is None);keys(tid,'ctrl+e');opened=ui(tid,lambda state:state.get('draft') and state.get('pending') is None)
