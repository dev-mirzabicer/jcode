...
try:
    ...
    assert e['type'] == 'done', e
    assert len(posts) - before == 7, (len(posts) - before, events[-10:])
    ...
    for flag in ['--json', '--ndjson']:
        ...
        if flag == '--json':
            ...
            for line in text.splitlines():
                json.loads(line)
        assert len(posts) - before == 7, (flag, len(posts) - before)
    ...
    assert 'PARENT NATIVE DONE' in text and len(posts) - before == 7
    ...
    try:
        ...
        json.loads(text)
        assert len(posts) - before == 7, 'autostart did not execute the complete child workflow'
