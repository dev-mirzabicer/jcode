...
pub fn run_find(root: &Path, args: &FindArgs) -> FindResult {
    run_find_controlled(root, args, &ExecutionControl::default())
        .expect("Uncontrolled search cannot be cancelled")
}
pub fn run_find_controlled(
    root: &Path,
    args: &FindArgs,
    ...
    for file in collect_file_entries_controlled(&scope, control)? {
        let relative_lower = file.relative_path.to_ascii_lowercase();
        if !has_path_evidence(&query_lower, &query_tokens, &relative_lower) {
            continue;
        }

        let Some(text) = read_text_file_controlled(&file.path, control)? else {
            continue;
        };
