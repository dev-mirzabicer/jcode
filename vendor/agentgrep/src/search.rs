...
use crate::workspace::{SearchScope, collect_file_entries_controlled, read_text_file_controlled};
use regex::Regex;
use serde::Serialize;
...
pub fn run_grep(root: &Path, args: &GrepArgs) -> Result<GrepResult, String> {
    run_grep_controlled(root, args, &ExecutionControl::default())
}
pub fn run_grep_controlled(
    root: &Path,
    args: &GrepArgs,
    control: &ExecutionControl,
) -> Result<GrepResult, String> {
    control.check()?;
    if let Some(result) = run_grep_with_rg(root, args, control)? {
        return Ok(result);
    }

    run_grep_native(root, args, control)
}
...
fn run_grep_native(
    ...
    let files = collect_file_entries_controlled(&scope, control)?;
    ...
    if worker_count <= 1 || files.len() <= 8 {
        for entry in files {
            ...
            if let Some(file_matches) = process_file(entry, &matcher, control) {
...
fn run_grep_with_rg(
    ...
    args: &GrepArgs,
    control: &ExecutionControl,
) -> Result<Option<GrepResult>, String> {
    if args.paths_only {
        let Some(paths) = run_rg_paths_only(root, args, control)? else {
            return Ok(None);
        };
        let files = paths
            .into_iter()
    ...
    let Some(match_map) = run_rg_match_map(root, args, control)? else {
        return Ok(None);
    };
    ...
    if worker_count <= 1 || matched_files.len() <= 8 {
        for (path, matches) in matched_files {
            ...
            if let Some(file_matches) = process_rg_match_file(root, path, matches, control) {
                total_matches += file_matches.matches.len();
                results.push(file_matches);
            }
        }
                ...
                handles.push(scope.spawn(move || {
                    ...
                    for (path, matches) in chunk.iter().cloned() {
                        ...
                        if let Some(file_matches) =
                            process_rg_match_file(root, path, matches, control)
                        {
                            partial_total += file_matches.matches.len();
                            partial.push(file_matches);
                        }
...
fn run_rg_paths_only(
    ...
    let output = match control.command(command) {
        Ok(output) => output,
        Err(err)
            if matches!(
                err.kind(),
...
fn run_rg_match_map(
    ...
    #[cfg(windows)]
    {
        let Some(output) = run_rg_json_search(root, args, control)? else {
            return Ok(None);
        };
    ...
    #[cfg(not(windows))]
    {
        if let Some(output) = run_rg_plain_search(root, args, control)? {
            match parse_rg_plain(&output.stdout, &matcher) {
                Ok(match_map) => return Ok(Some(match_map)),
                Err(_) => {
                    let Some(json_output) = run_rg_json_search(root, args, control)? else {
                        return Ok(None);
                    };
...
fn process_rg_match_file(
    ...
    let text = read_text_file_controlled(&absolute_path, control)
        .ok()
        .flatten()?;
...
fn process_file(
    ...
    let text = read_text_file_controlled(&entry.path, control)
        .ok()
        .flatten()?;

    let mut matches = Vec::new();
    for (idx, line) in text.lines().enumerate() {
        if idx.is_multiple_of(1024) && control.check().is_err() {
            return None;
        }
...
fn compact_match_line(line: &str, _match_span: Option<(usize, usize)>) -> String {
    line.to_string()
}
...
mod tests {
    ...
    impl crate::execution::ExecutionHost for NativeOnly {
        fn cancelled(&self) -> bool {
            self.checks
                .fetch_add(1, std::sync::atomic::Ordering::SeqCst)
                >= self.stop_after
        }
        fn command(&self, _: std::process::Command) -> std::io::Result<std::process::Output> {
            Err(std::io::Error::from(std::io::ErrorKind::Unsupported))
        }
    }
    #[test]
    fn controlled_native_search_preserves_results_and_observes_scan_cancellation() {
        let dir = tempdir().unwrap();
        for index in 0..30 {
            fs::write(
                dir.path().join(format!("source{index}.rs")),
                "fn needle() {}\n",
            )
            .unwrap();
        }
        let control = ExecutionControl::new(std::sync::Arc::new(NativeOnly {
            checks: Default::default(),
            stop_after: usize::MAX,
        }));
        let result = run_grep_controlled(dir.path(), &grep_args("needle"), &control).unwrap();
        assert_eq!(result.total_matches, 30);
        let control = ExecutionControl::new(std::sync::Arc::new(NativeOnly {
            checks: Default::default(),
            stop_after: 10,
        }));
        assert!(run_grep_controlled(dir.path(), &grep_args("needle"), &control).is_err());
    }
    use crate::cli::GrepArgs;
    use std::fs;
    ...
    #[test]
    fn grep_preserves_complete_huge_matched_lines() {
        let dir = tempdir().unwrap();
        ...
        assert_eq!(line_text, &huge_line);
    }

    #[test]
    fn rg_plain_parser_preserves_huge_lines() {
        ...
        assert_eq!(
            line_text,
            stdout
                .strip_prefix("assets/demo.json:1:100:")
                .unwrap()
                .trim_end_matches('\n')
        );
    }
