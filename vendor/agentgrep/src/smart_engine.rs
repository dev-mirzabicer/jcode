...
use crate::workspace::{
    SearchScope, TextFile, collect_file_entries_controlled, read_text_file_controlled,
};
use serde::Serialize;
use std::path::Path;
...
pub fn run_smart(root: &Path, query: &SmartQuery, args: &SmartArgs) -> Result<SmartResult, String> {
    run_smart_controlled(root, query, args, &ExecutionControl::default())
}
pub fn run_smart_controlled(
    root: &Path,
    query: &SmartQuery,
    args: &SmartArgs,
    ...
    for entry in collect_file_entries_controlled(&scope, control)? {
        ...
        let Some(text) = read_text_file_controlled(&entry.path, control)? else {
            continue;
        };
        let file = TextFile {
            path: entry.path,
...
struct RegionSource<'a> {
    file: &'a TextFile,
    items: &'a [StructureItem],
    lower_lines: &'a [String],
}
fn build_regions(
...
mod tests {
  ...
  "known_regions": [
    ...
    #[test]
    fn smart_region_bodies_preserve_complete_selected_lines() {
        let dir = tempdir().unwrap();
        ...
        assert_eq!(region.body, format!("function handle_auth() {{{giant}}}"));
    }
}
