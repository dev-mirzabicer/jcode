...
//! preserves complete selected lines. The embedding harness owns presentation
//! budgets; explicit result-count selection remains here.

use crate::cli::{FindArgs, GrepArgs, SmartArgs};
use crate::find::{FindFile, FindResult};
use crate::outline::OutlineResult;
use crate::search::{FileMatches, GrepResult};
use crate::smart_dsl::Relation;
use crate::smart_engine::{SmartFile, SmartRegion, SmartResult};

pub fn render_grep_output(
    result: &GrepResult,
    args: &GrepArgs,
...
fn render_grep_file(
    ...
    for group in &file.groups {
        ...
        let remaining_matches = state.remaining_matches();
...
/// Compatibility API: selected match lines remain complete for host presentation.
pub fn compact_rendered_match_line(line: &str, _args: &GrepArgs) -> String {
    line.to_string()
}

/// Compatibility API: selected region lines remain complete for host presentation.
pub fn compact_region_body_line(line: &str) -> String {
    line.to_string()
}

pub fn render_find_output(result: &FindResult, args: &FindArgs) -> String {
    if args.paths_only {
        return result
            .files
            .iter()
...
pub fn render_outline_output(result: &OutlineResult) -> String {
    let mut lines = vec![
...
pub fn render_smart_output(result: &SmartResult, args: &SmartArgs) -> String {
    if args.paths_only {
        return result
            .files
            .iter()
...
fn render_smart_region(region: &SmartRegion, debug_score: bool, lines: &mut Vec<String>) {
    ...
    // Region bodies are already line-capped by smart_engine at construction
    // time (see compact_region_body), so lines here are bounded. Re-applying
    // compact_region_body_line would mangle the engine's truncation marker.
...
mod tests {
    ...
    #[test]
    fn match_line_preserves_huge_lines() {
        let line = format!(
        ...
        let compact = compact_rendered_match_line(&line, &grep_args("set_status_notice"));
    ...
    #[test]
    fn compact_match_line_keeps_short_lines_verbatim() {
        let line = "fn set_status_notice() {}";
        assert_eq!(
            compact_rendered_match_line(line, &grep_args("set_status_notice")),
            line
        );
    }

    #[test]
    fn region_body_preserves_huge_lines() {
        let line = format!("function handleAuth(){{{}}}", "var x=1;".repeat(2000));

        let compact = compact_region_body_line(&line);

        assert_eq!(compact, line);

        let short = "fn small() {}";
        assert_eq!(compact_region_body_line(short), short);
    }
}
