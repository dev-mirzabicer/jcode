...
use crate::workspace::{
    SearchScope, collect_file_entries_controlled, normalize_display_path, read_text_file_controlled,
};
use serde::Serialize;
...
pub fn run_outline(root: &Path, args: &OutlineArgs) -> Result<OutlineResult, String> {
    run_outline_controlled(root, args, &ExecutionControl::default())
}
pub fn run_outline_controlled(
    root: &Path,
    args: &OutlineArgs,
    ...
    if !file_path.exists() {
        return Err(file_not_found_error(root, &args.file, &file_path, control));
    }
    if !file_path.is_file() {
        return Err(format!("not a file: {}", file_path.display()));
    }

    let text = read_text_file_controlled(&file_path, control)?
...
fn file_not_found_error(
    ...
    let suggestions = suggest_similar_files(root, requested, control);
...
fn suggest_similar_files(root: &Path, requested: &str, control: &ExecutionControl) -> Vec<String> {
    ...
    for entry in collect_file_entries_controlled(&scope, control).unwrap_or_default() {
