...
#[tokio::test]
async fn quiet_persistent_ws_cancellation_closes_request_and_invalidates_only_its_chain() {
    ...
    received.await.unwrap();drop(rx);
    let stopped=tokio::time::timeout(Duration::from_secs(2),&mut task).await;
    if stopped.is_err(){task.abort();server.abort();}
    assert!(matches!(stopped,Ok(Ok(PersistentWsResult::Success))),"Quiet cancellation must not wait for another event or become a retryable transport failure");
