...
impl ExecutionStore {
    pub fn register_runtime(&self, runtime: &RuntimeEndpoint) -> Result<()> {
        ...
        self.connection()?.execute("INSERT INTO runtimes (id,endpoint,auth_key,lease_path,protocol_version,process_id) VALUES (?1,?2,?3,?4,?5,?6)",
            params![runtime.id,runtime.endpoint.to_str().context("Non-UTF-8 runtime endpoint")?,runtime.auth_key,runtime.lease_path.to_str().context("Non-UTF-8 runtime lease")?,runtime.protocol_version,runtime.process_id])?;
        Ok(())
    }
    pub fn runtime_endpoint(&self, id: &str) -> Result<Option<RuntimeEndpoint>> {
        Ok(self.connection()?.query_row("SELECT endpoint,auth_key,lease_path,protocol_version,process_id FROM runtimes WHERE id=?1",[id],|row| {
            Ok(RuntimeEndpoint{id:id.to_string(),endpoint:PathBuf::from(row.get::<_,String>(0)?),auth_key:row.get(1)?,lease_path:PathBuf::from(row.get::<_,String>(2)?),protocol_version:row.get(3)?,process_id:row.get(4)?})
        }).optional()?)
    }
}
