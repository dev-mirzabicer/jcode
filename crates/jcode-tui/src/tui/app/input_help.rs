...
impl App {
    pub(super) fn command_help(&self, topic: &str) -> Option<String> {
        ...
        let help = match topic.as_str() {
            ...
            "memory" if crate::config::config().features.memory => {
            ...
            "startup" => {
                "/startup\nOpen the authoritative Startup Context editor. Browse or search the active project, add files or one exact external path, bulk-select a directory's direct non-directory children, reorder the unsaved draft, inspect current bounded previews, and lazily read every exact captured receipt chunk.\n\nUse u for `Use in this session` or p for `Use in this session + save as project default`. Jcode requests a fresh authoritative preview, binds external confirmation to each exact resolved target, then shows the model-route, session-history, pre-dispatch, late-addition, and future-only consequences before applying. Queued, applying, recovery, per-target success, failure, and cancellation remain inspectable with s.\n\nEsc unwinds confirmation, review, result, search, receipt, preview, and browser-directory layers before closing; q closes directly. The unsaved draft survives recoverable failures and reconnects."
            }
            "usage" => {
