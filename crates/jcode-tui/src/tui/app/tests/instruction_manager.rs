...
fn instruction_manager_remote_physical_enter_keys_and_replies_use_server_authority() {
    ...
    runtime.block_on(async {
        ...
        let snapshot=crate::protocol::InstructionInspectionSnapshot {snapshot:"authoritative".into(),session_id:"inspection-fixture".into(),active_agent:Some("global:fixture".into()),repositories:vec![],resources:crate::protocol::InstructionRowsPage {offset:0,total:1,next:None,rows:vec![crate::protocol::InstructionRow {description:String::new(),variants:Vec::new(),key:"server-resource".into(),id:"fixture".into(),name:"Server".into(),kind:"agent".into(),scope:"project".into(),repository:"project".into(),origin:crate::protocol::InstructionOrigin::Managed,effective:true,redefines_global:true,high_impact:true,valid:true,warning:None}]}};
